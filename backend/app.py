...
def evaluate(site_number):
    ...
    for h in [1, 6, 12, 24]:
        ...
        o3_r2 = r2_score(o3_actual, o3_pred) if len(o3_actual) > 1 and np.var(o3_actual) > 0 else None
        ...
        no2_r2 = r2_score(no2_actual, no2_pred) if len(no2_actual) > 1 and np.var(no2_actual) > 0 else None
    ...
    if site_number in WAQI_SITES:
        ...
        if token:
            try:
                ...
                print(f"⚠️  WAQI exception for site {site_number}: {e}")

    result = fetch_aqi_openmeteo(site_number)
    if result:
        result["from_cache"] = False
        _aqi_cache[site_number] = {**result, "cached_at": datetime.now().isoformat()}
        return result
...
def register_token():
    ...
    try:
        ...
        if existing:
            ...
            session.commit()
            return jsonify({'success': True, 'message': 'Token updated', 'token_id': existing.id})
        else:
            new_token = UserToken(
                fcm_token=fcm_token,
            ...
            session.commit()
            return jsonify({'success': True, 'message': 'Token registered', 'token_id': new_token.id})
    except Exception as e:
...
def update_preferences():
    ...
    try:
        ...
        session.commit()
        return jsonify({'success': True, 'preferences': token_record.to_dict()})
    except Exception as e:
        session.rollback()
        return jsonify({'error': str(e)}), 500
    finally:
...
def get_weather(site_number):
    ...
    try:
        ...
        return jsonify({
            ...
            "weather": {
                "temperature":    {"value": safe_get("temperature_2m"),       "unit": "°C"},
                "humidity":       {"value": safe_get("relativehumidity_2m"),   "unit": "%"},
                "wind_speed":     {"value": safe_get("windspeed_10m"),         "unit": "m/s"},
                "wind_direction": {"value": safe_get("winddirection_10m"),     "unit": "°"},
                "pressure":       {"value": safe_get("surface_pressure"),      "unit": "hPa"},
                "precipitation":  {"value": safe_get("precipitation"),         "unit": "mm"},
                "solar_radiation":{"value": safe_get("shortwave_radiation"),   "unit": "W/m²"},
                "uv_index":       {"value": safe_get("uv_index"),              "unit": ""},
                "boundary_layer": {"value": safe_get("boundary_layer_height"), "unit": "m"},
